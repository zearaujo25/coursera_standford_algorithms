...
def find_iteration_solution(graph, solution_array):
    iteration_solution = {}
    for node in graph:
        min_path_next_node = min([ solution_array[-1][incoming_edge[0]] + incoming_edge[2] for incoming_edge in graph[node]['incomming_edges']]) if len(graph[node]['incomming_edges']) >0 else float("inf")
